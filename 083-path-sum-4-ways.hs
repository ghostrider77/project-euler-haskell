...
updateDistances matrix n (i, j) distU distances t = go neighbours distances t
    where
        ...
        neighbours = filter validNode [(i - 1, j), (i + 1, j), (i, j + 1), (i, j - 1)]
...
shortestPath matrix n =  distances M.! bottomRight
    where
        (topLeft, bottomRight) = ((0, 0), (n - 1, n - 1))
...
main :: IO()
main = do
    content <- fmap lines (readFile "../resources/P0083.dat")
